import { atom } from 'recoil';

export const aroundShopListAtom = atom({
  key: 'aroundShopListAtom',
  default: [
    {
      name: '네일노마 연산역점',
      rating: '5.0',
      commentCount: 24,
      simpleAddress: '연산4번출구 4분, 시청8번출구 8분',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/643Pz/btri7REUlrB/LfT7BW72ggdUwans9P7Ps1/img_a375x375.jpg',
    },
    {
      name: '신지메이크업&amp;네일',
      rating: '4.8',
      commentCount: 20,
      simpleAddress: '연산역 4번출구로 나와 도보 3분거리',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bmhdi0/btqtCB0B99T/SGN7Jqd41uGkeZQcNb8Y4k/img_a375x375.jpg',
    },
    {
      name: '킴스네일카페 연산점',
      rating: '5.0',
      commentCount: 2,
      simpleAddress: '연산역 14번출구-100m 직진-그린조이 건물 5층',
      price: '네일 50,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/wR042/btquxwwzasS/uGLImxofMkkF5qWbGKNkVK/img_a375x375.jpg',
    },
    {
      name: '네일빛',
      rating: '5.0',
      commentCount: 51,
      simpleAddress: '연산동 홈플러스 옆 일동미라주더스타아파트 106동 상가',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bccDsZ/btqw0GcHWQ9/htqvVkNGRZp4q5fzxIl0pK/img_a375x375.jpg',
    },
    {
      name: '마벨리네일',
      rating: '4.7',
      commentCount: 78,
      simpleAddress: '부산 수영역 17번 출구에서 광안리 방향으로 올라오시면 도미노피자 골목.',
      price: '네일 22,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/dkndhM/btqAVFOnP0U/JuYpKvuKOypVHWL77QFGxk/img_a375x375.jpg',
    },
    {
      name: '크리스탈네일 양정점',
      rating: '4.9',
      commentCount: 135,
      simpleAddress: '양정역 2번출구 도보 5분이내 /양정 스타벅스 부근/양정로 5 2층',
      price: '네일 33,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/0nc91/btqtN84mCpX/Wh933B9hkpncuQxyUcJGS1/img_a375x375.jpg',
    },
    {
      name: '아띠네일',
      rating: '5.0',
      commentCount: 6,
      simpleAddress: '연산역,연산동홈플러스뒤 자이아파트 정문앞 ,더스타1층상가',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/YSrXu/btqy4ryogz8/IwQn4dsm2siF5gesorKmLk/img_a375x375.jpg',
    },
    {
      name: '어썸네일',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '수영역 2번출구 도보5분거리',
      price: '네일 38,500원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/mz3kr/btqwcIOzvP2/flB8o9bfpkAGubeo7ukojk/img_a375x375.jpg',
    },
    {
      name: '손더네일 서면점',
      rating: '4.8',
      commentCount: 213,
      simpleAddress: '서면 NC백화점 부근 (서면역 8번출구로 나와서 길건너 직진 )',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/LoFTh/btqt6DBCOH1/Lp5RR99LbabdBP8O8RhR1K/img_a375x375.jpg',
    },
    {
      name: '네일데스티니',
      rating: '4.9',
      commentCount: 56,
      simpleAddress: 'NC백화점 서면점 정문앞',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/dlTvRh/btqtMCDLIpQ/wkEVH0snokNGFVCbUbiZn1/img_a375x375.jpg',
    },
    {
      name: '뷰티온탑',
      rating: '4.9',
      commentCount: 45,
      simpleAddress: '동래지하철 도보 1분거리이에요.철인7호 3층으로 찾아오세요.',
      price: '네일 29,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/F7z9d/btqClmfEYdm/9XWbUd9kcKcKsQAL5VpnR0/img_a375x375.jpg',
    },
    {
      name: '겸스네일',
      rating: '4.9',
      commentCount: 21,
      simpleAddress: '쌍용플레티넘 옆집 도쿄가다 맞은편',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/nGyCg/btqt3guGWZa/dsHPbEpfs8ck25EnbM1hs1/img_a375x375.jpg',
    },
    {
      name: '네일끌리다 동래점',
      rating: '5.0',
      commentCount: 3,
      simpleAddress: '수안역 5번 출구에서 도보로 5분 소요',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bhAioH/btqtVdQEstD/1f6E5znqRk4YzqcRhLj5g0/img_a375x375.jpg',
    },
    {
      name: '누니끄뷰티살롱 양정점',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '양정 스타벅스 맞은편 세븐일레븐건물 지하',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/didg6I/btqwAYqM8v8/Tpk0KRU6OxsmqiP688f460/img_a375x375.jpg',
    },
    {
      name: '힛더제이 광안점',
      rating: '5.0',
      commentCount: 4,
      simpleAddress: '광안리 스타벅스비치점 바로 뒤',
      price: '네일 29,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/n8xU3/btqvwQBB8Oc/LJ8gRGSCa8Yr0k9oSIoDek/img_a375x375.jpg',
    },
    {
      name: '아낙네일 명장안락본점',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '충렬초, 충렬고 사이 마포나루 맞은편',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/GYTqH/btqwVDAZuZ9/KG7is7k0DNNRKybCHxvNw0/img_a375x375.jpg',
    },
    {
      name: '아라누리네일앤뷰티',
      rating: '5.0',
      commentCount: 8,
      simpleAddress: '사직역 지하철3번 출구에서 3분거리입니다 ',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/covksw/btquiPPt2Zv/uNBzR0O3sOlnJxJKtCEMKK/img_a375x375.jpg',
    },
    {
      name: '네일바이쏘 서면점',
      rating: '5.0',
      commentCount: 13,
      simpleAddress: '전리단길 카페 이터널선샤인 맞은편, 카페룽고 건물 2층입니다',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/JqSxG/btqwdhqWEDu/DhHeR5WMF2L6RTC9jvwWLK/img_a375x375.jpg',
    },
    {
      name: '매료네일&amp;뷰티',
      rating: '5.0',
      commentCount: 5,
      simpleAddress: '롯데백화점 옆문 방향 범태네손짜장 건물 3층에 위치',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/ct7dPl/btqtLBstyh1/pzauF5dQnEnvYi2rS4ajp1/img_a375x375.jpg',
    },
    {
      name: '아이홀릭&amp;네일 광안점',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '부산 광안지하철역1번 출구 바로',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/K0ipe/btqyuTWxtbo/AneuI1k9gRKPzl9j3jsxi1/img_a375x375.jpg',
    },
    {
      name: '그레이스 네일 부산본점',
      rating: '4.9',
      commentCount: 42,
      simpleAddress: '전리단길내 위치 / 서면토요코인바로뒷편 대성빌딩맞은편 입구 2층 ',
      price: '네일 22,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/LbZfK/btqAlyCZJUW/N1UKQ9D4O8Okf7iMnRamJk/img_a375x375.jpg',
    },
    {
      name: '손더네일 서면2호점',
      rating: '4.9',
      commentCount: 121,
      simpleAddress: '서면 쥬디스태화 본관 옆 서브웨이 2층 ',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bnUOp5/btqAN919ofp/PifmB9SYBhaXb828lUSL0K/img_a375x375.jpg',
    },
    {
      name: '네일모나코',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '동래롯데백화점 주차장 맞은편 서울깍두기와 상품권 사이골목',
      price: '네일 80,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/b153p7/btqtMgtYWzE/DTZpoZRVbeyUjFmsNQ65bk/img_a375x375.jpg',
    },
    {
      name: '포쉬네일 부산본점',
      rating: '5.0',
      commentCount: 7,
      simpleAddress: '경성대부경대 지하철역 1분,대연리마크(몰) 2층(1층 유니클로,KFC)',
      price: '네일 49,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bgP6Ue/btqtM13PaKg/IWTrk94XSUfx6OXIo28Q21/img_a375x375.jpg',
    },
    {
      name: '네일리안 부산남구점',
      rating: '4.7',
      commentCount: 2,
      simpleAddress: '대연역 2번출구 경성대방향 도보 100m 제일은행버스정류장 051카페2층',
      price: '네일 25,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/fwaX9/btqt1Eoe4U7/vwcyUwUNuPKw5c3JIr2YIk/img_a375x375.jpg',
    },
    {
      name: '메르 정원 서면점',
      rating: '5.0',
      commentCount: 10,
      simpleAddress: '서면시장부근(서면1번가)',
      price: '네일 25,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/KfMay/btqvBEUcZAz/NhGg4KPOcC0JS0qfXwfBdk/img_a375x375.jpg',
    },
    {
      name: '드리밍뷰티 서면점',
      rating: '5.0',
      commentCount: 11,
      simpleAddress: '서면 롯데백화점 후문부근',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/c7V4mC/btqzexxZnx4/HnbWNns7VyYQdhJpfISSlK/img_a375x375.jpg',
    },
    {
      name: '미드나잇네일 연산점',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '시청역 도보 1분거리에 있습니다',
      price: '네일 44,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/hlvTt/btrfiG0kCIZ/LzaFjK396loSk219KlzRYK/img_a375x375.jpg',
    },
    {
      name: '쁘띠아이즈 센텀',
      rating: '5.0',
      commentCount: 2,
      simpleAddress: '해운대구 우동1520 롯데갤러리움 s동 124호 센텀시티역13번출구 ',
      price: '네일 15,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/eWM4d/btqC9zlFTHB/GsyMNz0E08rmA90nVmu4DK/img_a375x375.jpg',
    },
    {
      name: '또네일 부산연지점',
      rating: '4.5',
      commentCount: 2,
      simpleAddress: '연지동 부산은행 맞은편 노란 간판 입니다',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bfUVXL/btqwXWyYg8r/anQZiLo35EWXUBLGLsTAqk/img_a375x375.jpg',
    },
    {
      name: '손더네일 사직점',
      rating: '4.9',
      commentCount: 36,
      simpleAddress: '사직야구장 맞은편 자이언츠 파크 313호 ',
      price: '네일 15,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/d0FlAs/btqNCxjw1bi/HTgLLScK8AHi8CaHzvXQv1/img_a375x375.jpg',
    },
    {
      name: '그남자의네일공간 본점',
      rating: '5.0',
      commentCount: 3,
      simpleAddress: '지하철 2호선 금련산역 2번출구에서 2분거리에 있습니다.',
      price: '네일 25,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/cDybUS/btqOUln7zVH/ZbHYcdKWWNVLFB2agRhCG1/img_a375x375.jpg',
    },
    {
      name: '에이블링네일&amp;속눈썹 서면점',
      rating: '5.0',
      commentCount: 2,
      simpleAddress: '서면역2번출구 도보1분거리/ 서면지하상가6번출구 건물2층',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/QO43m/btqC2sr5XUJ/y82PnTxqESGGQqwKWmrv21/img_a375x375.jpg',
    },
    {
      name: '미대언니네일살롱',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '동래 롯데백화점 주차장 입구 바로 맞은편 3층 입니다 ',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bVhOYk/btqBGqo5Awx/AiA0YaKOzFgrck6RBlC881/img_a375x375.jpg',
    },
    {
      name: '네일바이손 양정점',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '양정역 5번 출구 백조 아파트 정문 옆 건물 1층',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/VKRcc/btqZQIf4yUa/HQ3TVyL55N31An356WIIUk/img_a375x375.jpg',
    },
    {
      name: '꼼꼼이네일&amp;속눈썹 안락점',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '스타벅스 안락DT점 맞은편 20m아래',
      price: '네일 15,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bYvnn1/btq6Ld8qtNZ/WajQLZ9eCOxmPBQ4p8IYk1/img_a375x375.jpg',
    },
    {
      name: '클래시 부산서면점',
      rating: '5.0',
      commentCount: 2,
      simpleAddress: '서면역 2번출구. 서면 ZARA 부근.',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bAjrXJ/btqF5fdq1eg/taZTSotY6kIYIVvW1oqeP0/img_a375x375.jpg',
    },
    {
      name: '델리카네일 광안점',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '광안자이아파트 건너편상가에있습니다!',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/Vkfcp/btq2JxvArrN/GisdY80OY2M6XoUsX86Fqk/img_a375x375.jpg',
    },
    {
      name: '나니뷰티 서면점',
      rating: '5.0',
      commentCount: 4,
      simpleAddress: '서면 엔씨백화점맞은편 골목 엄지척닭도리 맞은편 3층',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/dhXTkP/btqMwfrhPL5/ALPyhP8yQW8Jgux5ZkVBo0/img_a375x375.jpg',
    },
    {
      name: '미유(MeYou) 사직점',
      rating: '0.0',
      commentCount: 0,
      simpleAddress: '사직역2번출구에서 189m 걸어오시면 되세요:)',
      price: '네일 45,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/hWYEV/btq354q0txP/q7Lp5VDJKqCmfph3M72EKk/img_a375x375.jpg',
    },
    {
      name: '김언니뷰티',
      rating: '5.0',
      commentCount: 2,
      simpleAddress: '부전도서관 맞은편',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/iJI56/btqU4LAM97u/je1GQMjl1M5WV4tUtKFVJK/img_a375x375.jpg',
    },
    {
      name: '오늘엔네일 본점',
      rating: '5.0',
      commentCount: 7,
      simpleAddress: '부전도서관 맞은편',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/baAXp6/btq8xLVOL4m/kYZedO7ZO0llLhIFi8Vli0/img_a375x375.jpg',
    },
    {
      name: '살아있네일 서면점',
      rating: '4.9',
      commentCount: 27,
      simpleAddress: '서면 롯데백화점 롯데호텔 후문 부라더식당3층 ',
      price: '네일 11,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/csCHyU/btqE6AHHLEq/ZLIeZFk3JQfDQhBGl00zk0/img_a375x375.jpg',
    },
    {
      name: '사직동네일샵 본점',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '사직1치안센터 버스정류장,사직아동병원 옆, 부산은행 맞은편 2층',
      price: '네일 30,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/9BrJH/btqUYfVSIJx/wpkqAoQycV9hirLSV5w8k0/img_a375x375.jpg',
    },
    {
      name: '네일앤미 사직점',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '사직역1번출구1분거리/쌍둥이돼지국밥골목안',
      price: '네일 40,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bGJEhe/btq9tJhzVza/Bp5iL3894Qq124aDRShY40/img_a375x375.jpg',
    },
    {
      name: '랑만뷰티살롱 경성대점',
      rating: '4.9',
      commentCount: 10,
      simpleAddress: '경성대학교1번출구 도보2분, 커피스미스 오른쪽방면 2번째건물 3층',
      price: '네일 15,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/coFj8G/btqF7tnL6yZ/xy1baRH6PAoUoKDw3wJ7K0/img_a375x375.jpg',
    },
    {
      name: '비비드 사직동본점',
      rating: '5.0',
      commentCount: 5,
      simpleAddress: '사직역4번출구에서 미남방면 도보3분거리',
      price: '네일 29,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bWisDN/btq70ogmezI/UhVhkcalJKNxZXo4qeSZkK/img_a375x375.jpg',
    },
    {
      name: '슈퍼네일 서면점',
      rating: '5.0',
      commentCount: 2,
      simpleAddress: '경성대부경대역 2번 출구 / 버스 (07006) 경성대부경대역 하차 ',
      price: '네일 35,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bQdphm/btrobUDYZUE/KoFsEtDssQVZVVkwcU5Nc1/img_a375x375.jpg',
    },
    {
      name: '네일클로버 온천장역점',
      rating: '5.0',
      commentCount: 1,
      simpleAddress: '롯데백화점정문에서 가야방면으로250m직진 한솔폴라리스1층 상가 안쪽에위치',
      price: '네일 33,000원',
      thumbnailUrl:
        'https://mud-kage.kakao.com/dn/bcoAB2/btq3Fda0cJb/5Qr9xzNlqfu6ZM9tqzl1qK/img_a375x375.jpg',
    },
  ],
});
